d3.svg.legend = function() {
    ...
    function redraw() {
        ...
        if (orientation == "vertical") {
            g.selectAll("g.legendCells").select("text.breakLabels").style("font-size", "13px").style("display", "block").style("text-anchor", "start").attr("x", cellWidth + cellPadding+4).attr("y", 5 + (cellHeight / 2)).text(function(d) {return labelFormat(d.stop[0]) + (d.stop[1].length > 0 ? " - " + labelFormat(d.stop[1]) : "")})
